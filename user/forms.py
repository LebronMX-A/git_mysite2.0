    ...
    password = forms.CharField(label='密码',
                               widget=forms.PasswordInput(
                                   attrs={'class': 'form-control',
                                          'placeholder': '请输入用密码'}))  # PasswordInput是一个类，可以再其中加入属性

    # label为让前端页面变为label=""的值，widget=forms.PasswordInput设置输入密码是为密文
...
class RegisterForm(forms.Form):
    username = forms.CharField(label='用户名',
                               ...
                               min_length=3,
                               widget=forms.TextInput(
                                   attrs={'class': 'form-control', 'placeholder': '请输入3-30用户名'}))  # form-control输入框组
    ...
    confirm_password = forms.CharField(label='再输入密码',
                                       min_length=6,
                                       widget=forms.PasswordInput(
                                           attrs={'class': 'form-control',
                                                  'placeholder': '请输入用密码'}))
    ...
    def clean_confirm_password(self):
        ...
        if password != confirm_password:
            raise forms.ValidationError('两次密码输入不一样')  # 如果出错就抛出错误并且将错误返回，在html显示
        return password
...
class BindEmailForm(forms.Form):
    ...
    verification_code = forms.CharField(
        ...
        max_length=20,
        widget=forms.TextInput(
            attrs={'class': 'form-control', 'placeholder': '点击"发送验证嘛"发送到邮箱'}
        )
    )
